		...
		if ( PCDheader.data === 'ascii' ) {
			...
			for ( let i = 0, l = lines.length; i < l; i ++ ) {
				...
				if ( offset.rgba !== undefined ) {
					...
					const rgba_field_index = PCDheader.fields.findIndex( field => field === 'rgba' );
					const rgba_type = PCDheader.type[ rgba_field_index ];
					const float = parseFloat( line[ offset.rgba ] );
					let rgba = float;
					...
					if ( rgba_type === 'F' ) {
						...
						const farr = new Float32Array( 1 );
						farr[ 0 ] = float;
						rgba = new Int32Array( farr.buffer )[ 0 ];

					}

					const r = rgba >> 32 & 0x000000ff;
					const g = rgba >> 16 & 0x000000ff;
					const b = rgba >> 8 & 0x000000ff;
					color.push( r / 255, g / 255, b / 255 );
