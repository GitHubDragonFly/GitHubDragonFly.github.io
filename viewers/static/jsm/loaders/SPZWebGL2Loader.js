 ...
 * const data = await loader.loadAsync( './models/splat/example.spz' );
 * ```
 *
 * @augments Loader
 * @three_import import { SPZLoader } from 'path/to/SPZWebGL2Loader.js';
 */
...
function parseSPZAttributes( {
	...
	const scaleXYZ = new Float32Array( count * 3 );
	const quaternionXYZW = new Float32Array( count * 4 );

	for ( let i = 0; i < count; i ++ ) {

		const i3 = i * 3;
		const i4 = i * 4;
		const sx = SCALE_LUT[ scales[ i3 + 0 ] ];
		...
		quaternionXYZW[ i4 + 0 ] = quaternion[ 0 ]; // qx
		quaternionXYZW[ i4 + 1 ] = quaternion[ 1 ]; // qy
		quaternionXYZW[ i4 + 2 ] = quaternion[ 2 ]; // qz
		quaternionXYZW[ i4 + 3 ] = quaternion[ 3 ]; // qw

		colorBytes[ i4 + 0 ] = COLOR_LUT[ colors[ i3 + 0 ] ];
