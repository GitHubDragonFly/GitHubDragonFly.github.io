	...
	parse( text, path ) {
		...
		function buildMaterial( data ) {
			...
			for ( const key in parameters ) {
				...
				switch ( key ) {
					...
					case 'specular':
						...
						if ( parameter.texture ) material.specularMap = getTexture( parameter.texture, SRGBColorSpace );
						break;
			...
			if ( extra !== undefined && extra.technique !== undefined ) {
				...
				material.side = extra.technique.double_sided === 1 ? DoubleSide : FrontSide;

				if ( extra.technique.bump && extra.technique.bump.texture ) {

					material.normalMap = getTexture( extra.technique[ bump ].texture );
