 ...
 */
   ...
   * @param {string} extension - manually provided extension for Model (.pmd or .pmx) so it could be read from blob
   ...
   */
		...
		load( url, extension='', onLoad, onProgress, onError ) {
			...
			const modelExtension = (extension !== '') ? extension : this._extractExtension( url ).toLowerCase(); // Should I detect by seeing header?
   ...
   * @param {string} extension - extension of the Model (.pmd or .pmx) file
   * @param {function} onLoad
   * @param {function} onProgress
   * @param {function} onError
   */
		...
		loadWithAnimation( modelUrl, vmdUrl, extension, onLoad, onProgress, onError ) {

			const scope = this;
			this.load( modelUrl, extension, function ( mesh ) {
