...
Notes about three.js viewers:
 ...
 - buttons, where applicable: `E` - edges, `F` - flatShading, `K` - kinematics, `A` - animations, `X`- morphs, `V` - variants, `T` - textures, `VC` - vertex colors, `R` - reflectivity, `P` - poses, `L` - lines (ldraw), `CS` - construction step (ldraw), `OS` - material side (original/front/back/double), `#` - grid, `RST` - reset
