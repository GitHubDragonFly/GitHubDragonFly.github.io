...
- Special notes about `3D Tiles Viewer`:
  ...
  - These viewers are specifically designed to stream 3D Tiles:
    - Support for v1.1 ImplicitTiling, MultipleContents, Property Tables and all kinds of Metadata, by performing conversion to explicit:
      ...
      - `3d-tiles-renderer` additionally supports v1.0 Batch Table and most formats (not sure about `.cmpt` format)
      - `threedtiles` currently does not support v1.0 Batch Table or either of `.i3dm`, `.pnts` and `.cmpt` formats
...
- Buttons, where applicable:
  ...
  - `g` - glass globe with snow flakes (currently available in desktop GLTF, USD and PLY+STL viewers only)
