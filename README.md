...
Notes about three.js viewers:
 ...
 - buttons, where applicable: `E` - edges, `F` - flatShading, `K` - kinematics, `A` - animations, `X`- morphs, `V` - variants, `T` - textures, `P` - poses, `CS` - Construction Step, `DS` - side (front/back/double), `#` - grid, `RST` - reset
